document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMessage(msg, isMe) {
        ...
        let badgeHTML = '';
        if (msg.isAdmin || msg.username.toLowerCase() === 'tmunn') {
        ...
        const innerHTML = `
                <div class="w-7 h-7 shrink-0 rounded-full font-bold flex items-center justify-center text-[10px] border border-white/5 ${isTmunn || msg.isAdmin ? 'bg-[#1a1a1a] text-[#FFD700] border-[#FFD700] shadow-[0_0_10px_rgba(255,215,0,0.5)]' : isMe ? 'bg-indigo-600 text-white' : msg.isBot ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/30 text-sm' : 'bg-zinc-800 text-zinc-300'} mt-0.5 relative overflow-hidden">
                    ${avatarContent}
                </div>
    ...
    function updateActiveUsers(users) {
        ...
        users.forEach(user => {
            ...
            if (user.isAdmin || user.username.toLowerCase() === 'tmunn') {
